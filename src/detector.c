...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, float thresh, float iou_thresh, int mjpeg_port, int show_imgs, int benchmark_layers, char* chart_path)
{
    ...
    while (get_current_iteration(net) < net.max_batches) {
        ...
        */
        ...
        int calc_map_for_each = 4 * train_images_num / (net.batch * net.subdivisions);  // calculate mAP for each Epoch
        if (net.max_batches != calc_map_for_each*50) {  // total 200
            // net.steps = 1;
            printf(net.steps);
            printf("\n Should train %d iterations", calc_map_for_each*50);
            net.max_batches = calc_map_for_each*50;
        }
        ...
        if (calc_map) {
            ...
            if (mean_average_precision > 0) printf("\n Last accuracy mAP@0.5:0.95 = %2.2f %%, best = %2.2f %% ", mean_average_precision * 100, best_map * 100);  // iou_thresh,
        }
